/* VIEW_PORTFOLIO_DESIRED_FACILITIES
 * Diese View gibt alle möglichen Kontonummern wieder, die uns für das Tape interessieren.
 */

-- VIEW erstellen
------------------------------------------------------------------------------------------------------------------------
drop view CALC.VIEW_PORTFOLIO_DESIRED_FACILITIES;
create or replace view CALC.VIEW_PORTFOLIO_DESIRED_FACILITIES as
with
...
--         -- Grundgesammtheit
--         select BRANCH_FACILITY, FACILITY_ID, PORTFOLIO as PORTFOLIO
--         from AMC.VIEW_PORTFOLIO_BASIC_SELECTION as FACILITIES
--         inner join CURRENT_CUTOFFDATE            as COD      on COD.CUT_OFF_DATE between FACILITIES.VALID_FROM_DATE and coalesce(FACILITIES.VALID_TO_DATE,CURRENT_DATE)
--         union all
        -- Grundgesammtheit & Extra Konten
        select distinct BRANCH_FACILITY, FACILITY_ID, PORTFOLIO as PORTFOLIO
        from CALC.SWITCH_PORTFOLIO_FACILITY_MANUAL_CURRENT   as FACILITIES
        inner join CURRENT_CUTOFFDATE                        as COD      on COD.CUT_OFF_DATE between FACILITIES.VALID_FROM_DATE and coalesce(FACILITIES.VALID_TO_DATE,CURRENT_DATE)
    ),
    ...
    PORTFOLIO_SELECTION as (
        select distinct
            BRANCH_FACILITY,
            FACILITY_ID,
            CALC.MAP_FUNC_PORTFOLIO_TO_ROOT(first_value(PORTFOLIO) over (partition by FACILITY_ID, BRANCH_FACILITY order by PORTFOLIO DESC nulls last)) as PORTFOLIO_ROOT_DESC,
            CALC.MAP_FUNC_PORTFOLIO_TO_ROOT(first_value(PORTFOLIO) over (partition by FACILITY_ID, BRANCH_FACILITY order by PORTFOLIO ASC nulls last)) as PORTFOLIO_ROOT_ASC
    ...
    DESIRED_FACILITIES_REPORTED as (
        select
            ...
            case
                when PORTFOLIO_ROOT_DESC like 'Murex%' then -- Murex only if no other
                    PORTFOLIO_ROOT_ASC
                else
                    PORTFOLIO_ROOT_DESC
            end as PORTFOLIO_ROOT
        from PORTFOLIO_SELECTION
    ),
    -- hinzufügen der Geko Doppelkunden
    DESIRED_FACILITY_SURROGATE as (
        select
            ...
            BASE.FACILITY_ID_REPORT                                 as FACILITY_ID_REPORT,
...
------------------------------------------------------------------------------------------------------------------------
call STG.TEST_PROC_BACKUP_AND_DROP('AMC','TABLE_PORTFOLIO_DESIRED_FACILITIES_CURRENT');
create table AMC.TABLE_PORTFOLIO_DESIRED_FACILITIES_CURRENT like CALC.VIEW_PORTFOLIO_DESIRED_FACILITIES distribute by hash(FACILITY_ID) in SPACE_AMC_A,SPACE_AMC_B,SPACE_AMC_C,SPACE_AMC_D,SPACE_AMC_E,SPACE_AMC_F;
create index AMC.INDEX_TABLE_PORTFOLIO_DESIRED_FACILITIES_CURRENT_FACILITY_ID  on AMC.TABLE_PORTFOLIO_DESIRED_FACILITIES_CURRENT (FACILITY_ID);
call STG.TEST_PROC_LOAD_AND_DROP_BACKUP_FOR('AMC','TABLE_PORTFOLIO_DESIRED_FACILITIES_CURRENT');
------------------------------------------------------------------------------------------------------------------------
...
-- SWITCH erstellen
------------------------------------------------------------------------------------------------------------------------
call STG.TEST_PROC_DROP_AND_CREATE_SWITCH('AMC','TABLE_PORTFOLIO_DESIRED_FACILITIES_CURRENT');
------------------------------------------------------------------------------------------------------------------------